 ...
 * <ul>
 * <li><code>ticketRegistry</code> - The Ticket Registry to maintain the list of
 * available tickets.</li>
 * <li><code>serviceTicketRegistry</code> - Provides an alternative to configure
 * separate registries for TGTs and ST in order to store them in different
 * locations (i.e. long term memory or short-term)</li>
 ...
 * <li><code>serviceTicketUniqueTicketIdGenerator</code> - Plug in to generate
 * unique secure ids for ServiceTickets.</li>
 * <li><code>ticketGrantingTicketExpirationPolicy</code> - The expiration policy
 * for TicketGrantingTickets.</li>
...
public final class CentralAuthenticationServiceImpl implements CentralAuthenticationService {
	...
	/**
	 * New Ticket Registry for storing and retrieving services tickets. Can point to
	 * the same one as the ticketRegistry variable.
	 */
	@NotNull
	...
	/**
	 * Attempts to sanitize the array of credentials by removing all null elements
	 * from the collection.
	 *
	 * @param credentials
	 *            credentials to sanitize
	...
	/**
	 * {@inheritDoc} Destroy a TicketGrantingTicket and perform back channel logout.
	...
	public ServiceTicket grantServiceTicket(final String ticketGrantingTicketId, final Service service,
		...
		if (sanitizedCredentials.size() > 0) {
			currentAuthentication = this.authenticationManager
					.authenticate(sanitizedCredentials.toArray(new Credential[]{}));
		...
		// Perform security policy check by getting the authentication that satisfies
		...
		final String ticketPrefix = authentications.size() == 1
				? ServiceTicket.PREFIX
				: ServiceTicket.PROXY_TICKET_PREFIX;
		...
		logger.info("Granted ticket [{}] for service [{}] for user [{}]", serviceTicket.getId(), service.getId(),
				principal.getId());
	...
	public TicketGrantingTicket delegateTicketGrantingTicket(final String serviceTicketId,
		...
		if (!registeredService.getProxyPolicy().isAllowedToProxy()) {
			logger.warn("ServiceManagement: Service [{}] attempted to proxy, but is not allowed.",
		...
		final String pgtId = this.ticketGrantingTicketUniqueTicketIdGenerator
				.getNewTicketId(TicketGrantingTicket.PROXY_GRANTING_TICKET_PREFIX);
		final TicketGrantingTicket proxyGrantingTicket = serviceTicket.grantTicketGrantingTicket(pgtId, authentication,
	...
	public Assertion validateServiceTicket(final String serviceTicketId, final Service service) throws TicketException {
		...
		try {
			...
			final Authentication authentication = getAuthenticationSatisfiedByPolicy(root,
	...
	public TicketGrantingTicket createTicketGrantingTicket(final Credential... credentials)
		...
		if (sanitizedCredentials.size() > 0) {
			// 认证
			// deployerConfigContext.xml 配置了 authenticationManager
			// AuthenticationManager 用于验证用户
			final Authentication authentication = this.authenticationManager.authenticate(credentials);
	...
	public <T extends Ticket> T getTicket(final String ticketId, final Class<? extends Ticket> clazz)
		...
		if (ticket == null) {
			logger.debug("Ticket [{}] by type [{}] cannot be found in the ticket registry.", ticketId,
					clazz.getSimpleName());
