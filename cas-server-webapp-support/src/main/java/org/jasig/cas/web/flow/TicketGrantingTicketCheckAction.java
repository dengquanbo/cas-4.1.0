...
public class TicketGrantingTicketCheckAction extends AbstractAction {
	 ...
	 *
	 * @throws Exception
	 *             in case ticket cannot be retrieved from the service layer
	 * @return {@link #NOT_EXISTS}, {@link #INVALID}, or {@link #VALID}.
	 */
	@Override
	protected Event doExecute(final RequestContext requestContext) throws Exception {
		// 1.获取第一步放入 scope 中的 ticketGrantingTicketId
		...
		try {
			// 2. 存在 TGT，则根据 TGT 获取 ticket
			final Ticket ticket = this.centralAuthenticationService.getTicket(tgtId, Ticket.class);
			// 2.1 ticket 存在且未过期说明 TGT 有效
