...
/**
 * Performs a basic check if an authentication request for a provided service is
 * authorized to proceed based on the registered services registry configuration
 * (or lack thereof).
 *
...
public final class ServiceAuthorizationCheck extends AbstractAction {
	...
	protected Event doExecute(final RequestContext context) throws Exception {
		// 获得 FlowScope 中的 service
		final Service service = WebUtils.getService(context);
		// No service == plain /login request. Return success indicating transition to
		// the login form
		// 不存在 service，直接返回成功
		if (service == null) {
			return success();
		...
		// 配置的 RegisteredService 是否有存在一个匹配service
