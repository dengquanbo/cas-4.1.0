...
public final class InitialFlowSetupAction extends AbstractAction {
	...
	protected Event doExecute(final RequestContext context) throws Exception {
		...
		// 第一次调用该方法，会执行if的逻辑，以后由于pathPopulated为true了，将跳过。
		...
		// 往 request scope, flow scope 放入键值对 key = ticketGrantingTicketId,value = cookie
		// 的值
		WebUtils.putTicketGrantingTicketInScopes(context,
				this.ticketGrantingTicketCookieGenerator.retrieveCookieValue(request));

		// 往 flow scope 放入键值对 key = warnCookieValue, value = cookie 的值
		WebUtils.putWarningCookie(context, Boolean.valueOf(this.warnCookieGenerator.retrieveCookieValue(request)));

		// 解析 service，这里会调用 SimpleWebApplicationServiceImpl 的 createServiceFrom
        ...
        // 往 flow scope 放入键值对 key = service, value = service
	...
	/**
	 * Decide whether CAS should allow authentication requests when no service is
	 * present in the request. Default is enabled.
	 *
