...
public class AuthenticationViaFormAction {
	...
	/**
	 * Authentication succeeded with warnings from authn subsystem that should be
	 * displayed to user.
	 */
	...
	/**
	 * Tries to to determine if the login ticket in the request flow scope matches
	 * the login ticket provided by the request. The comparison is case-sensitive.
	 *
	 * @param context
	 *            the context
	...
	protected boolean checkLoginTicketIfExists(final RequestContext context) {
		...
		// FlowScope 与 RequestScope 的 loginTicket 是否相同
	...
	protected boolean isRequestAskingForServiceTicket(final RequestContext context) {
		// 获得 ticketGrantingTicketId
		final String ticketGrantingTicketId = WebUtils.getTicketGrantingTicketId(context);
		// 获得 service
		final Service service = WebUtils.getService(context);

		// 从请求参数获得 renew 不为空，service 不为空，ticketGrantingTicketId 不为空。则说明是重新获取
		// ServiceTicket
	...
	/**
	 * Create ticket granting ticket for the given credentials. Adds all warnings
	...
	protected Event createTicketGrantingTicket(final RequestContext context, final Credential credential,
			final MessageContext messageContext) {
		try {
			// 根据用户名和密码创建 TicketGrantingTicket
			// applicationContext 配置了 centralAuthenticationService
			final TicketGrantingTicket tgt = this.centralAuthenticationService.createTicketGrantingTicket(credential);
			// 往 request scope, flow scope 放入键值对 key = ticketGrantingTicketId,value =
			// tgt.getId 的值
	...
	/**
	 * New event based on the id, which contains an error attribute referring to the
	 * exception occurred.
	 *
	 * @param id
	 *            the id
	 ...
	 *            the ticket registry. No longer needed as the core service layer
	 *            returns the correct object type. Will be removed in future
	 *            versions.
